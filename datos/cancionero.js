/* ============================================================================
   ARTEFACTO GENERADO. No editar a mano.
   ----------------------------------------------------------------------------
   Generado por herramientas/generar.html
   Fuente : texto pegado
   Fecha  : 2026-07-23
   Items  : 57

   Para corregir una errata: edita el .txt de origen y volve a generar.
   Si editas este archivo a mano y te comes una coma, el domingo hay
   pantalla en blanco.
   ========================================================================== */

const CANCIONERO = {
  coleccion: "Cancionero",
  prefijo: "c",
  items: [

    {
      numero: 1,
      tipo: "cancion",
      titulo: "Dios Está Aquí",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Dios está aquí",
          "(Aleluya)",
          "Tan cierto como el aire que respiro,",
          "Tan cierto como la mañana se levanta,",
          "Tan cierto como que le canto y me puede oír."
        ]}
      ]
    },

    {
      numero: 2,
      tipo: "cancion",
      titulo: "Somos el Pueblo de Dios",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Somos el pueblo de Dios,",
          "Somos un pueblo especial,",
          "Llamados para anunciar,",
          "Las virtudes de aquel,",
          "Que nos llamó a su luz.",
          "Somos el pueblo de Dios,",
          "Su sangre nos redimió",
          "Y su Espíritu dio,",
          "Para darnos poder",
          "Y ser testigos de Él.",
          "Y llevaremos su gloria,",
          "A cada pueblo y nación.",
          "Trayendoles esperanza",
          "Y nuevas de salvación",
          "Y su amor nos impulsa,",
          "No nos podemos callar.",
          "Anunciaremos al mundo,",
          "De su amor y verdad."
        ]}
      ]
    },

    {
      numero: 3,
      tipo: "cancion",
      titulo: "Salmo 37",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Por Jehová son ordenados",
          "Los pasos del hombre,",
          "Y Él aprueba su camino,",
          "Cuando el hombre cayere no quedará postrado,",
          "Porque Jehová sostiene, sostiene su mano.",
          "Encomienda a Jehová, tu camino y confía en Él y Él hará,",
          "Guarda silencio ante Jehová,",
          "Y espera en Él y Él hará.",
          "Joven fui y he envejecido",
          "Y no he visto justo desamparado.",
          "Ni su descendencia",
          "Que mendigue pan,",
          "Porque Jehová sostiene,",
          "Sostiene su mano.",
          "Encomienda a Jehová, tu camino y confía en Él y Él hará,",
          "Guarda silencio ante Jehová y espera en Él y Él hará."
        ]}
      ]
    },

    {
      numero: 4,
      tipo: "cancion",
      titulo: "Sendas Dios Hará",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Sendas Dios hará, donde piensas que no hay,",
          "Él obra en maneras que,",
          "No podemos entender.",
          "Él me guiará, a su lado estaré,",
          "Amor y fuerza me dará,",
          "Un camino hará",
          "Donde no lo hay.",
          "Sendas Dios hará, donde piensas que no hay,",
          "Él obra en maneras que,",
          "No podemos entender.",
          "Él me guiará, a su lado estaré,",
          "Amor y fuerza me dará,",
          "Un camino hará",
          "Donde no lo hay.",
          "Por camino en la soledad,",
          "Me guiará,",
          "Agua en el desierto encontraré.",
          "La tierra pasará, más su palabra eterna es.",
          "Él hará, algo nuevo hoy."
        ]}
      ]
    },

    {
      numero: 5,
      tipo: "cancion",
      titulo: "Fuerte Dios",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Fuerte Dios, consolador,",
          "Príncipe de paz.",
          "Gran Señor, vencedor,",
          "Siempre vives y siempre reinarás.",
          "Yo te exalto, Dios te exalto.",
          "Eres todo para mí,",
          "La creación canta de Ti.",
          "Yo te exalto, Dios te exalto,",
          "Te exalto Cristo mi Rey.",
          "Fuerte Dios, consolador,",
          "Príncipe de paz.",
          "Gran Señor, vencedor,",
          "Siempre vives y siempre reinarás.",
          "Yo te exalto, Dios te exalto.",
          "Eres todo para mí,",
          "La creación canta de Ti.",
          "Yo te exalto, Dios te exalto,",
          "Te exalto Cristo mi Rey.",
          "Te exalto Cristo mi Rey.",
          "Te exalto Cristo mi Rey.",
          "Te exalto Cristo mi Rey."
        ]}
      ]
    },

    {
      numero: 6,
      tipo: "cancion",
      titulo: "Al Estar",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Al estar en la presencia",
          "De tu divinidad,",
          "Y al contemplar la hermosura de tu santidad,"
        ]},
        { tipo: "coro", lineas: [
          "Mi Espíritu se alegra",
          "En tu majestad.",
          "Te adoro a tí, te adoro a tí."
        ]},
        { tipo: "estrofa", lineas: [
          "Cuando veo la grandeza",
          "De tu dulce amor,",
          "Y compruebo la pureza",
          "De tu corazón,"
        ]},
        { tipo: "coro", lineas: [
          "Mi Espíritu se alegra",
          "En tu majestad.",
          "Te adoro a tí, te adoro a tí."
        ]},
        { tipo: "estrofa", lineas: [
          "Y al estar aquí, delante de Ti te adoraré.",
          "Postrado ante Ti, mi corazón te adora, oh Dios.",
          "Y siempre quiero estar para adorar y contemplar tu santidad,",
          "Te adoro a Ti Señor,",
          "Te adoro a tí.",
          "Y al estar aquí, delante de Ti te adoraré.",
          "Postrado ante Ti, mi corazón te adora, oh Dios.",
          "Y siempre quiero estar para adorar y contemplar tu santidad,",
          "Te adoro a Ti Señor,",
          "Te adoro a tí."
        ]}
      ]
    },

    {
      numero: 7,
      tipo: "cancion",
      titulo: "Cerca de Ti",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Si decidiera negar mi fe,",
          "Y no confiar nunca más en Él,"
        ]},
        { tipo: "coro", lineas: [
          "No tengo a donde ir,",
          "No tengo a donde ir."
        ]},
        { tipo: "estrofa", lineas: [
          "Si despreciara en mi corazón,",
          "La santa gracia que me salvó,"
        ]},
        { tipo: "coro", lineas: [
          "No tengo a donde ir,",
          "No tengo a donde ir."
        ]},
        { tipo: "estrofa", lineas: [
          "Convencido estoy que sin tu amor se acabarían mis fuerzas.",
          "Y sin Ti mi corazón sediento se muere, se seca.",
          "Cerca de Ti, yo quiero estar,",
          "De tu presencia no me quiero alejar.",
          "Cerca de Ti Jesús, yo quiero estar,",
          "De tu presencia no me quiero alejar.",
          "Convencido estoy que sin tu amor se acabarían mis fuerzas.",
          "Y sin Ti mi corazón sediento se muere, se seca."
        ]}
      ]
    },

    {
      numero: 8,
      tipo: "cancion",
      titulo: "¿A Quién Iré en Necesidad?",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "¿A quién iré en necesidad?",
          "¿A quién iré en busca de paz?",
          "¿Y quién podrá mi vida saciar de verdad?",
          "¿Quién más tendrá de mi compasión,",
          "Y entenderá mi corazón?",
          "¿Quién cambiará mi eternidad sino Tú, Jesús?",
          "Cristo, ¿a dónde más podría ir?",
          "Cristo, ¿qué otro lugar puede existir?",
          "Sólo Tú tienes palabras de amor,",
          "Camino al Padre y verdad eres Tú.",
          "Cristo, ¿a dónde más podría ir?"
        ]}
      ]
    },

    {
      numero: 9,
      tipo: "cancion",
      titulo: "De Tal Manera Me Amó",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "De tal manera me amó,",
          "Que su vida no escatimó,",
          "Hasta el final Él se entregó y a la muerte fue porque Él me amó.",
          "De tal manera me amó",
          "Y no hay forma en que podré pagar,",
          "El precio de su grande amor,",
          "Pero toda mi alma quiero dar."
        ]},
        { tipo: "coro", lineas: [
          "Por su amor yo viviré",
          "De su amor yo cantaré",
          "Con mi Jesús caminaré,"
        ]},
        { tipo: "estrofa", lineas: [
          "Porque Él me amó a mí."
        ]},
        { tipo: "coro", lineas: [
          "Por su amor yo viviré",
          "De su amor yo cantaré",
          "Con mi Jesús caminaré,"
        ]},
        { tipo: "estrofa", lineas: [
          "Porque Él murió por mí,",
          "Le seguiré."
        ]}
      ]
    },

    {
      numero: 10,
      tipo: "cancion",
      titulo: "Dios Es Mi Conductor",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "coro", lineas: [
          "Dios es mi conductor,",
          "Él dirige mi vida",
          "Todas las situaciones",
          "Dependen de Él.",
          "Dios es mi conductor,",
          "Él dirige mi vida",
          "Todas las situaciones",
          "Dependen de Él."
        ]},
        { tipo: "estrofa", lineas: [
          "Verdaderamente su presencia,",
          "Me seguirá todos mis días",
          "Y en su casa yo viviré,",
          "Por largo tiempo,",
          "Un largo tiempo."
        ]},
        { tipo: "coro", lineas: [
          "Dios es mi conductor,",
          "Él dirige mi vida",
          "Todas las situaciones",
          "Dependen de Él.",
          "Dios es mi conductor,",
          "Él dirige mi vida",
          "Todas las situaciones",
          "Dependen de Él."
        ]},
        { tipo: "estrofa", lineas: [
          "Aunque ande en sombras,",
          "Cerca de la muerte,",
          "Si voy de su mano seguro estaré.",
          "Me darás aliento,",
          "Doblarás mis fuerzas",
          "Y andarás conmigo marcando el camino.",
          "Verdaderamente su presencia me seguirá todos mis días",
          "Y en su casa yo viviré,",
          "Por largo tiempo,",
          "Un largo tiempo.",
          "Dios es mi conductor."
        ]}
      ]
    },

    {
      numero: 11,
      tipo: "cancion",
      titulo: "Esa Paz Que Siento en Mi Alma",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Esa paz que siento en mi alma,",
          "No es porque todo me va bien.",
          "Esa paz que siento en mi alma,",
          "Es porque yo sigo a quién es fiel."
        ]},
        { tipo: "coro", lineas: [
          "No miro circunstancias, no, no,",
          "Yo miro su amor,",
          "No me guío por vista,",
          "Alegre voy.",
          "No miro circunstancias, no, no,",
          "Yo miro su amor,",
          "No me guío por vista,",
          "Alegre voy."
        ]},
        { tipo: "estrofa", lineas: [
          "Y aunque la higuera no florezca,",
          "Ni en las vides haya frutos,",
          "Y aunque los montes se lancen al mar,",
          "Y la tierra tiemble,",
          "Tengo en quién confiar."
        ]},
        { tipo: "coro", lineas: [
          "No miro circunstancias, no, no,",
          "Yo miro su amor,",
          "No me guío por vista,",
          "Alegre voy.",
          "No miro circunstancias, no, no,",
          "Yo miro su amor,",
          "No me guío por vista,",
          "Alegre voy."
        ]}
      ]
    },

    {
      numero: 12,
      tipo: "cancion",
      titulo: "Este Pueblo Que Has Formado",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Este pueblo que has formado hoy viene a entregar",
          "Las ofrendas de sus labios puestas en tu altar.",
          "Este pueblo que es tu iglesia hoy viene a adorar",
          "Al único, al amado,",
          "Al Rey de libertad.",
          "Y canta Hosanna, gloria Aleluya,",
          "Oh Jesús amado",
          "Tu amor me ha cautivado",
          "Y canta Hosanna, gloria Aleluya,",
          "Alabanzas hoy yo traigo",
          "A tu nombre Jesús."
        ]}
      ]
    },

    {
      numero: 13,
      tipo: "cancion",
      titulo: "Cuán Bello Es el Señor",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Cuán bello es el Señor,",
          "Cuán hermoso es el Señor.",
          "Cuán bello es el Señor,",
          "Hoy le quiero adorar.",
          "Cuán bello es el Señor,",
          "Cuán hermoso es el Señor.",
          "Cuán bello es el Señor,",
          "Hoy le quiero adorar.",
          "La belleza de mi Señor,",
          "Nunca se agotará,",
          "La hermosura de mi Señor,",
          "Siempre resplandecerá.",
          "La belleza de mi Señor,",
          "Nunca se agotará,",
          "La hermosura de mi Señor,",
          "Siempre resplandecerá."
        ]}
      ]
    },

    {
      numero: 14,
      tipo: "cancion",
      titulo: "Jesús, Señor de la Creación",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Jesús, Señor de la creación,",
          "Siendo en forma de Dios,",
          "Se despojó de sí mismo.",
          "Tomó, la semejanza de hombre y siendo puro y sin manchas,",
          "Entre nosotros vivió.",
          "Y a sí mismo se humilló,",
          "Tomando forma de siervo,",
          "Hasta su vida entregar",
          "Y en una cruz terminar.",
          "Más Dios, a lo sumo lo exaltó y su nombre engrandeció.",
          "Para que ante su autoridad,",
          "Toda rodilla se doble",
          "Y toda lengua confiese que Jesús es el Señor.",
          "Más Dios, a lo sumo lo exaltó y su nombre engrandeció.",
          "Para que ante su autoridad,",
          "Toda rodilla se doble",
          "Y toda lengua confiese que Jesús es el Señor."
        ]}
      ]
    },

    {
      numero: 15,
      tipo: "cancion",
      titulo: "Qué Maravilla Es Tener una Familia",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Que maravilla es tener una familia,",
          "Una familia en Cristo Jesús.",
          "Que maravilla es tener una familia,",
          "Una familia en Cristo Jesús.",
          "Una familia unida,",
          "Una familia real,",
          "Una familia que ama,",
          "Una familia celestial.",
          "Una familia unida,",
          "Una familia real,",
          "Una familia que ama,",
          "Una familia celestial."
        ]}
      ]
    },

    {
      numero: 16,
      tipo: "cancion",
      titulo: "Fui Hecho Justo",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Fui hecho justo,",
          "Sin merecerlo.",
          "Toda mi culpa ya cubierta está.",
          "Toda mi deuda,",
          "Quedó saldada,",
          "En la persona del Cordero",
          "Inmortal."
        ]},
        { tipo: "coro", lineas: [
          "Salvación, camino angosto,",
          "Plenitud de vida eterna, don de Dios.",
          "Una cruz, cubierta en sangre,",
          "Fue necesaria para nuestra redención."
        ]},
        { tipo: "estrofa", lineas: [
          "Regenerado y transformado,",
          "Cambié mi mente y mi forma de actuar.",
          "Hoy sólo anhelo, servir a Cristo,",
          "Y ser la luz, la sal, de un claro manantial."
        ]},
        { tipo: "coro", lineas: [
          "Salvación, camino angosto,",
          "Plenitud de vida eterna, don de Dios.",
          "Una cruz, cubierta en sangre,",
          "Fue necesaria para nuestra redención."
        ]},
        { tipo: "estrofa", lineas: [
          "Santificado soy, por Jesucristo,",
          "Me acerco a Él y obtengo más consagración.",
          "El enemigo, ya está vencido.",
          "Hoy demos gloria a nuestro Dios porque Él triunfó."
        ]},
        { tipo: "coro", lineas: [
          "Salvación, camino angosto,",
          "Plenitud de vida eterna, don de Dios.",
          "Una cruz, cubierta en sangre,",
          "Fue necesaria para nuestra redención."
        ]}
      ]
    },

    {
      numero: 17,
      tipo: "cancion",
      titulo: "Él Es Mi Paz",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Él es mi paz,",
          "Ha borrado todos mis temores,",
          "Él es mi paz, Él es mi paz.",
          "Él es mi paz,",
          "Ha borrado todos mis temores,",
          "Él es mi paz, Él es mi paz.",
          "Hecho toda mi ansiedad",
          "Sobre Él,",
          "Pues Él cuida de mi,",
          "Él es mi paz, Él es mi paz.",
          "Hecho toda mi ansiedad",
          "Sobre Él,",
          "Pues Él cuida de mi,",
          "Él es mi paz, Él es mi paz."
        ]}
      ]
    },

    {
      numero: 18,
      tipo: "cancion",
      titulo: "Te Bendeciré",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Te bendeciré,",
          "En la tierra de plenitud,",
          "Donde fluye la leche y miel,",
          "Te bendeciré.",
          "Te bendeciré, cuando en tierra desierta esté",
          "Y si en selva caminaré,",
          "Te bendeciré.",
          "Por bendiciones recibidas te alabaré,"
        ]},
        { tipo: "coro", lineas: [
          "Si tinieblas me rodean aún diré:",
          "Bendito sea el nombre de Dios,",
          "Bendito sea su nombre,",
          "Bendito sea el nombre de Dios,",
          "Bendito sea su glorioso nombre."
        ]},
        { tipo: "estrofa", lineas: [
          "Te bendeciré,",
          "Cuando el sol brille sobre mí y si todo marcha muy bien,",
          "Te bendeciré.",
          "Te bendeciré,",
          "Cuando en medio de prueba esté y aunque cueste seguir en pie,",
          "Te bendeciré.",
          "Por bendiciones recibidas",
          "Te alabaré,"
        ]},
        { tipo: "coro", lineas: [
          "Si tinieblas me rodean aún diré:",
          "Bendito sea el nombre de Dios,",
          "Bendito sea su nombre,",
          "Bendito sea el nombre de Dios,",
          "Bendito sea su glorioso nombre."
        ]},
        { tipo: "estrofa", lineas: [
          "Tú quitas y tu das,",
          "Tú quitas y Tú das.",
          "Mi corazón dirá:",
          "Bendito sea Dios.",
          "Bendito sea el nombre de Dios,",
          "Bendito sea su nombre.",
          "Bendito sea el nombre de Dios,",
          "Bendito sea su glorioso nombre."
        ]}
      ]
    },

    {
      numero: 19,
      tipo: "cancion",
      titulo: "Me Has Tomado en Tus Brazos",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Me has tomado en tus brazos y me has dado salvación,",
          "De tu amor has derramado en mi corazón.",
          "No sabré agradecerte,",
          "Lo que has hecho por mí,",
          "Solo puedo darte ahora mi",
          "Corazón."
        ]},
        { tipo: "coro", lineas: [
          "Gracias, gracias Señor,",
          "Gracias mi Señor Jesús.",
          "Gracias, muchas gracias Señor,",
          "Gracias mi Señor Jesús."
        ]},
        { tipo: "estrofa", lineas: [
          "En la cruz diste tu vida,",
          "Entregaste todo allí,",
          "Vida eterna regalaste al morir.",
          "Por tu sangre tengo entrada ante el trono celestial",
          "Puedo entrar confiadamente ante Ti.",
          "Para darte las"
        ]},
        { tipo: "coro", lineas: [
          "Gracias, gracias Señor,",
          "Gracias mi Señor Jesús.",
          "Gracias, muchas gracias Señor,",
          "Gracias mi Señor Jesús."
        ]}
      ]
    },

    {
      numero: 20,
      tipo: "cancion",
      titulo: "Celebrad a Cristo",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Celebrad a Cristo celebrad.",
          "Celebrad a Cristo celebrad.",
          "Celebrad a Cristo celebrad.",
          "Celebrad a Cristo celebrad.",
          "Resucitó, resucitó,",
          "Él vive para siempre.",
          "Resucitó, resucitó."
        ]},
        { tipo: "coro", lineas: [
          "¡Vamos a celebrar!",
          "¡Sí!"
        ]},
        { tipo: "coro", lineas: [
          "¡Vamos a celebrar!",
          "¡Sí!"
        ]},
        { tipo: "estrofa", lineas: [
          "¡Vamos a celebrar!",
          "Resucitó el Señor."
        ]}
      ]
    },

    {
      numero: 21,
      tipo: "cancion",
      titulo: "Hay Libertad",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Las cadenas de la muerte me quitó,",
          "Para mí ya no hay condenación.",
          "Hay libertad, libertad.",
          "Siendo esclavo",
          "Del pecado me libró,",
          "En la cruz compró mi redención y libertad, libertad."
        ]},
        { tipo: "coro", lineas: [
          "Rompió mis cadenas,",
          "Me dió vida nueva",
          "Y al fondo del mar echó mi maldad,",
          "Lavó mis pecados",
          "Y me ha perdonado,",
          "Solo en el nombre de Jesús,",
          "Hay libertad."
        ]},
        { tipo: "estrofa", lineas: [
          "Yo proclamo al que en la cruz por mí murió",
          "Y cambió mi duro corazón.",
          "Por libertad, libertad.",
          "El poder de su evangelio me salvó y no me avergüenzo del Señor.",
          "Mi libertad, libertad."
        ]},
        { tipo: "coro", lineas: [
          "Rompió mis cadenas,",
          "Me dió vida nueva",
          "Y al fondo del mar echó mi maldad,",
          "Lavó mis pecados",
          "Y me ha perdonado,",
          "Solo en el nombre de Jesús,",
          "Hay libertad."
        ]},
        { tipo: "estrofa", lineas: [
          "Al que el Hijo, hace libre,",
          "Es libre, en verdad.",
          "Al que el Hijo, hace libre,",
          "Es libre, en verdad."
        ]},
        { tipo: "coro", lineas: [
          "Rompió mis cadenas,",
          "Me dió vida nueva",
          "Y al fondo del mar echó mi maldad,",
          "Lavó mis pecados",
          "Y me ha perdonado,",
          "Solo en el nombre de Jesús,",
          "Hay libertad."
        ]}
      ]
    },

    {
      numero: 22,
      tipo: "cancion",
      titulo: "Alzaré Mis Ojos a los Montes",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Alzaré mis ojos a los montes ¿de dónde vendrá mi socorro?",
          "Mi socorro viene del Señor,",
          "Que hizo los cielos y la tierra.",
          "No dará tu pie al resbaladero,",
          "Ni se dormirá el que te guarda,",
          "He aquí, no se adormecerá,",
          "Ni se dormirá, el que guarda tu alma.",
          "Es tu guardador,",
          "Es tu sombra a la mano derecha.",
          "El sol no te fatigará de día, ni la luna de noche.",
          "El Señor te guardará de todo mal.",
          "Él guardará tu alma.",
          "El Señor guardará tu salida y tu entrada,",
          "Desde ahora y para siempre y para siempre."
        ]}
      ]
    },

    {
      numero: 23,
      tipo: "cancion",
      titulo: "Esperar en Ti",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Esperar en Ti,",
          "Difícil se que es.",
          "Mi mente dice no,",
          "No es posible.",
          "Pero mi corazón,",
          "Confiado está en Ti.",
          "Tú siempre has sido fiel,",
          "Me has sostenido.",
          "Y esperaré, pacientemente,",
          "Aunque la duda me atormente,",
          "Yo no confío con la mente,",
          "Lo hago con el corazón.",
          "Y esperaré, en la tormenta,",
          "Aunque tardare tu respuesta,",
          "Yo confiaré en tu providencia,",
          "Tu siempre tienes el control."
        ]}
      ]
    },

    {
      numero: 24,
      tipo: "cancion",
      titulo: "Gracias por la Cruz",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Gracias por la cruz oh Dios,",
          "El precio que pagaste por mi,",
          "Llevando mi pecado allí,",
          "Sublime amor, tu gracia me salvó.",
          "Gracias por tu amor oh Dios,",
          "Tus manos clavadas por mi,",
          "Me has lavado oh Señor",
          "Conozco hoy tu abrazo y tu perdón.",
          "Digno es el Señor,",
          "En su trono está,",
          "Hoy te coronamos Rey",
          "Y reinas con poder.",
          "Altísimo Señor,",
          "Jesús Hijo de Dios.",
          "El amado del cielo",
          "En la cruz murió,",
          "Digno es el Señor.",
          "Digno es el Señor.",
          "Digno es el Señor."
        ]}
      ]
    },

    {
      numero: 25,
      tipo: "cancion",
      titulo: "La Única Razón",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "La única razón de mi adoración,",
          "Eres Tú mi Jesús.",
          "El único motivo para vivir,",
          "Eres Tú mi Señor.",
          "Mi única verdad está en Ti,",
          "Eres mi luz y mi salvación.",
          "Mi único amor eres tu Señor y por siempre te alabaré.",
          "Tu eres todo poderoso,",
          "Eres grande y majestuoso,",
          "Eres fuerte, invencible,",
          "Y no hay nadie como Tú.",
          "Tu eres todo poderoso,",
          "Eres grande y majestuoso,",
          "Eres fuerte, invencible,",
          "Y no hay nadie como Tú."
        ]}
      ]
    },

    {
      numero: 26,
      tipo: "cancion",
      titulo: "La Gente Camina",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "La gente camina, cabizbaja y triste.",
          "Sus ojos perdidos, en la inmensidad y entre tantas voces",
          "Y entre tanta gente",
          "Y entre tantas luces,",
          "No le han visto a Él.",
          "Él también camina, como todos ellos,",
          "Un rostro radiante, en la multitud,",
          "Y pocos lo notan,",
          "Más cuando lo notan,",
          "Detienen su marcha",
          "Y se van tras Él.",
          "Así poco a poco, un pueblo se forma,",
          "Que vive en el mundo, pero no es de el,",
          "Que tiene problemas,",
          "Como todos ellos,",
          "Pero no parece,",
          "Padecer por ellos.",
          "Este pueblo canta, cuando el mundo llora,",
          "Y cuando está en sombras,",
          "Este pueblo es luz.",
          "Porque todos ellos,",
          "Claramente han visto,",
          "La luz que manaba,",
          "Del rostro de Cristo.",
          "Porque todos ellos,",
          "Claramente han visto,",
          "La luz que manaba,",
          "Del rostro de Cristo."
        ]}
      ]
    },

    {
      numero: 27,
      tipo: "cancion",
      titulo: "De Gloria en Gloria",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "De gloria en gloria te veo,",
          "Cuanto más te conozco",
          "Quiero saber más de Ti.",
          "Mi Dios, cual buen alfarero,",
          "Quebrántame, transfórmame,",
          "Moldeame a Tú imagen Señor.",
          "Quiero ser más como Tú,",
          "Ver la vida como Tú,",
          "Saturarme de tu Espíritu",
          "Y reflejar al mundo Tú amor.",
          "Quiero ser más como Tú,",
          "Ver la vida como Tú,",
          "Saturarme de tu Espíritu",
          "Y reflejar al mundo Tú amor."
        ]}
      ]
    },

    {
      numero: 28,
      tipo: "cancion",
      titulo: "Salvador a Ti Me Rindo",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Salvador a Ti me rindo",
          "Y obedezco sólo a Ti.",
          "Mi guiador, mí fortaleza,",
          "Todo encuentra mi alma en Ti."
        ]},
        { tipo: "coro", lineas: [
          "Yo me rindo a Ti,",
          "Yo me rindo a Ti,",
          "Mis flaquezas y pecados,",
          "Todo traigo a Ti."
        ]},
        { tipo: "estrofa", lineas: [
          "Te confiesa sus delitos,",
          "Mi contrito corazón.",
          "¡Oye, oh Cristo! Mi plegaria,",
          "Quiero en Ti tener perdón."
        ]},
        { tipo: "coro", lineas: [
          "Yo me rindo a Ti,",
          "Yo me rindo a Ti,",
          "Mis flaquezas y pecados,",
          "Todo traigo a Ti."
        ]},
        { tipo: "estrofa", lineas: [
          "A tus pies yo deposito,",
          "Mis riquezas, mi placer.",
          "Que Tú Espíritu me llene",
          "Y de Ti sienta el poder."
        ]},
        { tipo: "coro", lineas: [
          "Yo me rindo a Ti,",
          "Yo me rindo a Ti,",
          "Mis flaquezas y pecados,",
          "Todo traigo a Ti."
        ]},
        { tipo: "estrofa", lineas: [
          "Tú bondad será la historia,",
          "Que predique por doquier,",
          "Y tu amor inagotable,",
          "Será siempre mi querer."
        ]},
        { tipo: "coro", lineas: [
          "Yo me rindo a Ti,",
          "Yo me rindo a Ti,",
          "Mis flaquezas y pecados,",
          "Todo traigo a Ti."
        ]},
        { tipo: "estrofa", lineas: [
          "¡Oh, qué gozo encuentro en Cristo!",
          "¡Cuánta paz a mi alma da!",
          "Yo a su causa me consagro,",
          "Y su amor, mi amor será."
        ]},
        { tipo: "coro", lineas: [
          "Yo me rindo a Ti,",
          "Yo me rindo a Ti,",
          "Mis flaquezas y pecados,",
          "Todo traigo a Ti."
        ]}
      ]
    },

    {
      numero: 29,
      tipo: "cancion",
      titulo: "Crucificado Fue",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Tiempo atrás su hogar dejó,",
          "El gran Dios eternal,",
          "Naciendo en un establo,",
          "Su vida vino a dar.",
          "Vivió errante y sin lugar,",
          "En donde descansar,",
          "Buscando nuestras almas,",
          "Su vida vino a dar."
        ]},
        { tipo: "coro", lineas: [
          "Crucificado fue,",
          "Más ha resucitado,",
          "Y ahora Él me defiende,",
          "Victoria Él ganó.",
          "Crucificado fue,",
          "Más la tumba,",
          "Con poder venció,",
          "Y aquel corazón que roto fue,",
          "Hoy late otra vez."
        ]},
        { tipo: "estrofa", lineas: [
          "Pensaste alguna vez que Él,",
          "Tomaba Tú lugar,",
          "Al apurar la copa que,",
          "Cargaba Tú pecar.",
          "El día que Dios señaló,",
          "Su sangre Cristo dio,",
          "Su Padre lo desamparó,",
          "Cuando tus pecados Él llevó."
        ]},
        { tipo: "coro", lineas: [
          "Crucificado fue,",
          "Más ha resucitado,",
          "Y ahora Él me defiende,",
          "Victoria Él ganó.",
          "Crucificado fue,",
          "Más la tumba,",
          "Con poder venció,",
          "Y aquel corazón que roto fue,",
          "Hoy late otra vez."
        ]}
      ]
    },

    {
      numero: 30,
      tipo: "cancion",
      titulo: "El Dios Que Adoramos",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "El Dios que hizo los cielos y la tierra",
          "Con el poder de su palabra reina con autoridad.",
          "El Dios que aún los vientos le obedecen una palabra es suficiente",
          "Para los muertos levantar."
        ]},
        { tipo: "coro", lineas: [
          "Nadie es como Él,",
          "Oh gran yo soy.",
          "Tú eres el Dios que adoramos,",
          "Todopoderoso y soberano,",
          "Grande en misericordia y poder para salvar.",
          "Tú eres el Dios que adoramos,",
          "Quien derrotó la muerte y el pecado,",
          "Glorioso redentor y Rey, te adoramos."
        ]},
        { tipo: "estrofa", lineas: [
          "El Dios que descendió",
          "Desde su trono",
          "Para llevar sobre sus hombros nuestra culpa y transgresión.",
          "Jesús, exaltado sobre todo,",
          "Nombre sobre todo nombre",
          "Sólo en Él hay salvación."
        ]},
        { tipo: "coro", lineas: [
          "Nadie es como Él,",
          "Oh gran yo soy.",
          "Tú eres el Dios que adoramos,",
          "Todopoderoso y soberano,",
          "Grande en misericordia y poder para salvar.",
          "Tú eres el Dios que adoramos,",
          "Quien derrotó la muerte y el pecado,",
          "Glorioso redentor y Rey, te adoramos."
        ]},
        { tipo: "estrofa", lineas: [
          "A Él sea la gloria y el poder,",
          "Todo es de Él y para Él.",
          "A Él sea la gloria y el poder,",
          "Todo es de Él y para Él.",
          "Tú eres el Dios que adoramos,",
          "Todopoderoso y soberano,",
          "Grande en misericordia y poder para salvar.",
          "Tú eres el Dios que adoramos,",
          "Quien derrotó la muerte y el pecado,",
          "Glorioso redentor y Rey, te adoramos."
        ]}
      ]
    },

    {
      numero: 31,
      tipo: "cancion",
      titulo: "Desde el Fondo del Mar",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Desde el fondo del mar,",
          "(Desde el fondo del mar)",
          "Del más alto lugar,",
          "(Del más alto lugar)",
          "Del más hondo abismo,",
          "(Del más hondo abismo).",
          "Te alabaré.",
          "Desde mi corazón,",
          "(Desde mi corazón)",
          "Fuerte o débil esté,",
          "(Fuerte o débil esté)",
          "Cada instante de mi vida,",
          "(Cada instante de mi vida)",
          "Con gozo yo diré.",
          "Que Tú eres la roca eterna,",
          "Señor sobre cielo y tierra Tú eres Dios.",
          "Que toda criatura adore,",
          "Que toda nación se postre a Ti Señor,",
          "Tú eres Dios."
        ]}
      ]
    },

    {
      numero: 32,
      tipo: "cancion",
      titulo: "Renuévame, Señor Jesús",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Renuévame, Señor Jesús,",
          "Ya no quiero ser igual.",
          "Renuévame, Señor Jesús,",
          "Pon en mí tu corazón.",
          "Porque todo lo que hay,",
          "Dentro de mí,",
          "Necesita ser cambiado, Señor.",
          "Porque todo lo que hay,",
          "Dentro de mí corazón,",
          "Necesita más de Ti.",
          "Necesita más de Ti.",
          "Necesita más de Ti."
        ]}
      ]
    },

    {
      numero: 33,
      tipo: "cancion",
      titulo: "Cada Cosa en la Vida",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Cada cosa en la vida,",
          "Tiene su justo lugar,",
          "Cada cosa tiene un tiempo,",
          "Para hacerse realidad.",
          "Es por eso que preciso",
          "Que me enseñes a mirar.",
          "Lo que tengo por delante,",
          "Y el presente que está acá."
        ]},
        { tipo: "coro", lineas: [
          "Enseñame a vivir el hoy de tal manera que mañana, no tenga que",
          "Reprocharme el ayer.",
          "Enseñame a vivir el hoy de tal manera que mañana, no tenga que",
          "Reprocharme el ayer.",
          "Enseñame"
        ]},
        { tipo: "estrofa", lineas: [
          "Tiempo de nacer,",
          "Tiempo de morir,",
          "Tiempo de llorar,",
          "También de reír,",
          "Tiempo de buscar,",
          "Tiempo de perder,",
          "Tiempo de saber lo que hay que hacer.",
          "Es por eso que preciso,",
          "Que me enseñes a vivir",
          "Y a no equivocarme,",
          "Cuando tenga que elegir."
        ]},
        { tipo: "coro", lineas: [
          "Enseñame a vivir el hoy de tal manera que mañana, no tenga que",
          "Reprocharme el ayer.",
          "Enseñame a vivir el hoy de tal manera que mañana, no tenga que",
          "Reprocharme el ayer.",
          "Enseñame"
        ]}
      ]
    },

    {
      numero: 34,
      tipo: "cancion",
      titulo: "Contempla a Dios",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "¿Quién formó las olas al hablar?",
          "¿Quién contó la arena en el mar?",
          "Reyes, pueblos, tiemblan a su voz la creación le rinde adoración."
        ]},
        { tipo: "coro", lineas: [
          "Contempla a Dios,",
          "En su trono está,",
          "Venid y adoradle.",
          "Contempla al Rey,",
          "Nadie es como Él,",
          "Venid y adoradle."
        ]},
        { tipo: "estrofa", lineas: [
          "¿Quién le dio consejo al Señor?",
          "¿Quién cuestiona lo que Él habló?",
          "¿Quién al sabio Dios puede enseñar?",
          "¿Quién comprende su gran",
          "Majestad?"
        ]},
        { tipo: "coro", lineas: [
          "Contempla a Dios,",
          "En su trono está,",
          "Venid y adoradle.",
          "Contempla al Rey,",
          "Nadie es como Él,",
          "Venid y adoradle."
        ]},
        { tipo: "estrofa", lineas: [
          "¿Quién sufrió los clavos en dolor?",
          "¿Quién llevó el mal del pecador?",
          "Jesucristo, humilde al perecer,",
          "Ha triunfado y reina con poder."
        ]},
        { tipo: "coro", lineas: [
          "Contempla a Dios,",
          "En su trono está,",
          "Venid y adoradle.",
          "Contempla al Rey,",
          "Nadie es como Él,",
          "Venid y adoradle."
        ]},
        { tipo: "estrofa", lineas: [
          "¡Reinarás por siempre!",
          "¡Que tu gloria brille hoy!"
        ]},
        { tipo: "coro", lineas: [
          "Contempla a Dios,",
          "En su trono está,",
          "Venid y adoradle.",
          "Contempla al Rey,",
          "Nadie es como Él,",
          "Venid y adoradle."
        ]}
      ]
    },

    {
      numero: 35,
      tipo: "cancion",
      titulo: "Cuán Grande Es Dios",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "El esplendor de un Rey,",
          "Vestido en majestad,",
          "La tierra alegre está,",
          "La tierra alegre está.",
          "Cubierto esta de luz,",
          "Venció la oscuridad",
          "Y tiembla a su voz,",
          "Tiembla a su voz."
        ]},
        { tipo: "coro", lineas: [
          "Cuán grande es Dios,",
          "Cántale cuán grande es Dios y todos lo verán,",
          "Cuán grande es Dios."
        ]},
        { tipo: "estrofa", lineas: [
          "Día a día Él está",
          "Y el tiempo está en Él,",
          "Principio y el fin,",
          "Principio y el fin.",
          "La trinidad en Dios",
          "El Padre, Hijo, Espíritu,",
          "Cordero y el León,",
          "Cordero y el León."
        ]},
        { tipo: "coro", lineas: [
          "Cuán grande es Dios,",
          "Cántale cuán grande es Dios y todos lo verán,",
          "Cuán grande es Dios."
        ]},
        { tipo: "estrofa", lineas: [
          "Y tu nombre sobre todo es,",
          "Tu eres digno de alabar",
          "Y mi ser dirá,",
          "Cuán grande es Dios."
        ]},
        { tipo: "coro", lineas: [
          "Cuán grande es Dios,",
          "Cántale cuán grande es Dios y todos lo verán,",
          "Cuán grande es Dios."
        ]}
      ]
    },

    {
      numero: 36,
      tipo: "cancion",
      titulo: "Él Es la Voz",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Él es la voz que mis oídos abrió,",
          "La libertad que mis cadenas rompió.",
          "Él es la luz que mi alma oscura iluminó,",
          "Él es Jesús toda mi vida",
          "Transformó.",
          "Él hizo cambiar mi odio en amor,",
          "Convierte en alegría mi dolor.",
          "Puede transformar al más",
          "Pecador,",
          "Darle una nueva vida y su perdón.",
          "Él hizo que vuelva a latir mi corazón.",
          "Él es la voz que mis oídos abrió,",
          "La libertad que mis cadenas rompió.",
          "Él es la luz que mi alma oscura iluminó,",
          "Él es Jesús toda mi vida",
          "Transformó.",
          "Él hizo cambiar mi odio en amor,",
          "Convierte en alegría mi dolor.",
          "Puede transformar al más",
          "Pecador,",
          "Darle una nueva vida y su perdón.",
          "Él hizo que vuelva a latir mi corazón."
        ]}
      ]
    },

    {
      numero: 37,
      tipo: "cancion",
      titulo: "Hay Uno Que Me Ama",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Hay uno que me ama,",
          "Que a mi lado siempre está,",
          "Y camina junto a mí, su Hijo yo soy.",
          "Cuando en pruebas yo estoy,",
          "Se que Él me ayudará,",
          "No es solo una ilusión, su Hijo soy Dios es bueno para mí,",
          "Sé que Él conmigo está,",
          "Con Él yo puedo hablar,",
          "Se que me escuchará.",
          "Dios es bueno para mí,",
          "Sé que Él conmigo está,",
          "Con Él yo puedo hablar, su Hijo soy.",
          "Esperando estoy en Ti,",
          "Me sostienes con tu amor,",
          "Confiado puedo estar, tu Hijo soy.",
          "Cuando débil yo estoy,",
          "Dices “no te dejaré”,",
          "Me aseguras otra vez, tu Hijo soy."
        ]}
      ]
    },

    {
      numero: 38,
      tipo: "cancion",
      titulo: "En la Casa de Dios",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Mejor es un día en la casa de Dios.",
          "Que mil años lejos de Él.",
          "Prefiero un rincón en la casa de Dios.",
          "Que todo el palacio de un Rey.",
          "Que todo el palacio de un Rey."
        ]},
        { tipo: "coro", lineas: [
          "Ven conmigo a la casa de Dios.",
          "Celebraremos juntos su amor.",
          "Haremos fiesta en honor de aquel que nos amó.",
          "Estando aquí en la casa de Dios.",
          "Alegraremos su corazón.",
          "Le brindaremos ofrendas de obediencia y amor.",
          "En la casa de Dios oh oh oh.",
          "En la casa de Dios oh oh oh."
        ]},
        { tipo: "estrofa", lineas: [
          "Arde mi alma, arde de amor,",
          "Por aquel que me dió la vida.",
          "Por eso le anhela mi corazón,",
          "Anhela de su compañía.",
          "Anhela de su compañía."
        ]},
        { tipo: "coro", lineas: [
          "Ven conmigo a la casa de Dios.",
          "Celebraremos juntos su amor.",
          "Haremos fiesta en honor de aquel que nos amó.",
          "Estando aquí en la casa de Dios.",
          "Alegraremos su corazón.",
          "Le brindaremos ofrendas de obediencia y amor.",
          "En la casa de Dios oh oh oh.",
          "En la casa de Dios oh oh oh."
        ]},
        { tipo: "estrofa", lineas: [
          "En la casa de Dios."
        ]}
      ]
    },

    {
      numero: 39,
      tipo: "cancion",
      titulo: "Él Es el Rey",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Él es el Rey infinito en poder,",
          "Él es el Rey de los cielos.",
          "Seré para Él siervo fiel",
          "Pues mi vida compró con su amor.",
          "Él es el Rey lo confiesa mi ser,",
          "Él es el Rey de los siglos.",
          "Mi vida la rindo a sus pies,",
          "Él es el Rey sobre mi corazón.",
          "Él es el Rey, Él es el Rey,",
          "Él es el Rey de mi vida",
          "Él es el Rey, Él es el Rey,",
          "Reina con autoridad.",
          "Su reino eterno es,",
          "Su trono el cielo es.",
          "Él es el Rey que viene su pueblo a llevar."
        ]}
      ]
    },

    {
      numero: 40,
      tipo: "cancion",
      titulo: "Nuestro Canto Expresa un Sentir",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Nuestro canto expresa un sentir.",
          "Y es preciso que se pueda oír.",
          "En las calles, en las plazas,",
          "En los campos, las montañas,",
          "A lo largo y a lo ancho del país."
        ]},
        { tipo: "coro", lineas: [
          "Jesucristo es la verdad,",
          "Nuestra fe y nuestro compromiso.",
          "Él es la vida en plenitud,",
          "Él es la fuerza y es la luz,",
          "Es la alegría que me alienta a continuar.",
          "Es el poder para cambiar,",
          "Toda una vida y darle más.",
          "Jesucristo es la razón para vivir."
        ]},
        { tipo: "estrofa", lineas: [
          "Este es el momento para actuar.",
          "En cada rincón, cada lugar.",
          "Nuestra patria necesita,",
          "Actitudes decididas,",
          "De cristianos que se jueguen de verdad."
        ]},
        { tipo: "coro", lineas: [
          "Jesucristo es la verdad,",
          "Nuestra fe y nuestro compromiso.",
          "Él es la vida en plenitud,",
          "Él es la fuerza y es la luz,",
          "Es la alegría que me alienta a continuar.",
          "Es el poder para cambiar,",
          "Toda una vida y darle más.",
          "Jesucristo es la razón para vivir."
        ]}
      ]
    },

    {
      numero: 41,
      tipo: "cancion",
      titulo: "Si Estás Cansado de Buscar Sueños",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Si estás cansado de buscar sueños,",
          "Buscas alcanzar tus propias metas.",
          "Te atormentan dudas y problemas,",
          "Lleva a Jesús tus cargas hoy."
        ]},
        { tipo: "coro", lineas: [
          "A Jesús, a Jesús, a Jesús dale todo,",
          "Sueños rotos y un destrozado corazón.",
          "A Jesús, a Jesús, a Jesús dale todo,",
          "La tristeza en gozo Él cambiará."
        ]},
        { tipo: "estrofa", lineas: [
          "No promete solo días soleados,",
          "En tu vida habrán muchas",
          "Tormentas.",
          "Pero Él te ayudará con tu carga.",
          "Grande recompensa Él te dará."
        ]},
        { tipo: "coro", lineas: [
          "A Jesús, a Jesús, a Jesús dale todo,",
          "Sueños rotos y un destrozado corazón.",
          "A Jesús, a Jesús, a Jesús dale todo,",
          "La tristeza en gozo Él cambiará."
        ]}
      ]
    },

    {
      numero: 42,
      tipo: "cancion",
      titulo: "Jesucristo Basta",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Nuestros corazones,",
          "Insaciables son,",
          "Hasta que conocen",
          "A su Salvador."
        ]},
        { tipo: "coro", lineas: [
          "Tal y como somos nos amó,",
          "Hoy nos acercamos sin temor.",
          "Él es el agua que al beber,",
          "Nunca más tendremos sed.",
          "Jesucristo basta.",
          "Jesucristo basta.",
          "Mi castigo recibió",
          "Y su herencia me entregó.",
          "Jesucristo basta.",
          "Jesucristo basta."
        ]},
        { tipo: "estrofa", lineas: [
          "Fuimos alcanzados,",
          "Por su gran amor.",
          "Con brazos abiertos,",
          "Nos recibe hoy."
        ]},
        { tipo: "coro", lineas: [
          "Tal y como somos nos amó,",
          "Hoy nos acercamos sin temor.",
          "Él es el agua que al beber,",
          "Nunca más tendremos sed.",
          "Jesucristo basta.",
          "Jesucristo basta.",
          "Mi castigo recibió",
          "Y su herencia me entregó.",
          "Jesucristo basta.",
          "Jesucristo basta."
        ]},
        { tipo: "estrofa", lineas: [
          "Tal y como somos nos amó",
          "Hoy nos acercamos sin temor sin temor.",
          "Tal y como somos nos amó",
          "Hoy nos acercamos sin temor sin temor.",
          "Él es el agua que al beber,",
          "Nunca más tendremos sed.",
          "Jesucristo basta.",
          "Jesucristo basta.",
          "Mi castigo recibió",
          "Y su herencia me entregó.",
          "Jesucristo basta.",
          "Jesucristo basta.",
          "Jesucristo basta.",
          "Ahora hay un futuro",
          "Y esperanza fiel,",
          "En su amor confiamos,",
          "Hay descanso en Él."
        ]}
      ]
    },

    {
      numero: 43,
      tipo: "cancion",
      titulo: "Salmo 16",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Guárdame, oh Señor,",
          "Porque en tí he confiado.",
          "Oh alma mía, dijiste a Jehová:",
          "Tú eres mi Señor; no hay para mí bien fuera de Ti.",
          "Guárdame, oh Señor,",
          "Porque en tí he confiado.",
          "Oh alma mía, dijiste a Jehová:",
          "Tú eres mi Señor; no hay para mí bien fuera de Ti."
        ]}
      ]
    },

    {
      numero: 44,
      tipo: "cancion",
      titulo: "Al Único Que Es Digno",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Al único que es digno de recibir,",
          "Gloria y honra, fuerza y poder.",
          "Al Dios y Padre eternal,",
          "Invisible pero real,",
          "A Él hoy ministramos loor.",
          "Al único que es digno de recibir,",
          "Gloria y honra, fuerza y poder.",
          "Al Dios y Padre eternal,",
          "Invisible pero real,",
          "A Él hoy ministramos loor.",
          "Te coronamos",
          "Señor y Rey Jesús.",
          "Te coronamos",
          "Señor y Rey Jesús.",
          "Adoramos tu nombre,",
          "Nos rendimos a tus pies,",
          "Consagramos todo nuestro ser a Ti.",
          "Consagramos todo nuestro ser a Ti."
        ]}
      ]
    },

    {
      numero: 46,
      tipo: "cancion",
      titulo: "Aquel Que la Buena Obra Empezó",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "coro", lineas: [
          "Aquel que la buena obra empezó.",
          "(Aquel que buena obra empezó).",
          "Aquel que buena obra empezó.",
          "Será fiel en completarla,",
          "Él es fiel en completarla,",
          "El que la comenzó",
          "Será fiel en completarla en mí."
        ]},
        { tipo: "estrofa", lineas: [
          "Si la lucha que enfrentas a tu esperanza tornó,",
          "Ya sin par.",
          "O de tanto esperar ya perdiendo tu canción estás.",
          "Puedes creer, que su mano está en",
          "Tí, seguro estás, nunca te ha de abandonar, eres su gozo",
          "Y su gran tesoro aquí."
        ]},
        { tipo: "coro", lineas: [
          "Aquel que la buena obra empezó.",
          "(Aquel que buena obra empezó).",
          "Aquel que buena obra empezó.",
          "Será fiel en completarla,",
          "Él es fiel en completarla,",
          "El que la comenzó",
          "Será fiel en completarla en mí."
        ]}
      ]
    },

    {
      numero: 47,
      tipo: "cancion",
      titulo: "A Cristo Solo a Cristo",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "A Cristo solo a Cristo,",
          "Yo exaltaré.",
          "A Cristo solo a Cristo,",
          "Yo adoraré.",
          "Porque Él me ha dado vida eterna,",
          "Porque Él me ha dado el poder,",
          "Porque Él me ha dado la victoria,",
          "Él es mi Rey.",
          "A Cristo he proclamado Rey.",
          "A Cristo he proclamado Rey."
        ]}
      ]
    },

    {
      numero: 48,
      tipo: "cancion",
      titulo: "Aquel Que Nos Amó",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Aquel que nos amó, testigo fiel exaltaré.",
          "Al que es principio y fin,",
          "Al gran yo soy",
          "Adoraré.",
          "Aquel que nos amó, testigo fiel exaltaré.",
          "Al que es principio y fin,",
          "Al gran yo soy",
          "Adoraré."
        ]},
        { tipo: "coro", lineas: [
          "A Jesús mi Señor,",
          "Resplandor de la gloria de Dios.",
          "A Jesús mi gran Rey",
          "Alabanza y honra daré."
        ]},
        { tipo: "estrofa", lineas: [
          "Siempre te amaré,",
          "Siempre te adoraré.",
          "Siempre te cantaré",
          "Siempre te exaltaré."
        ]},
        { tipo: "coro", lineas: [
          "A Jesús mi Señor,",
          "Resplandor de la gloria de Dios.",
          "A Jesús mi gran Rey",
          "Alabanza y honra daré."
        ]}
      ]
    },

    {
      numero: 49,
      tipo: "cancion",
      titulo: "Alza Tus Ojos y Mira",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Alza tus ojos y mira,",
          "La cosecha esta lista.",
          "El tiempo ha llegado,",
          "La mies está madura.",
          "Esfuérzate y sé valiente",
          "Levántate y predica,",
          "A todas las naciones",
          "Que Cristo es la vida."
        ]},
        { tipo: "coro", lineas: [
          "Y sera, llena la tierra de su gloria,",
          "Se cubrirá, como las aguas cubren la mar."
        ]},
        { tipo: "estrofa", lineas: [
          "No, no hay otro nombre,",
          "Dado a los hombres.",
          "Jesucristo, es el Señor"
        ]},
        { tipo: "coro", lineas: [
          "Y sera, llena la tierra de su gloria,",
          "Se cubrirá, como las aguas cubren la mar."
        ]},
        { tipo: "estrofa", lineas: [
          "Como las aguas cubren la mar.",
          "Como las aguas cubren la mar.",
          "Como las aguas cubren la mar."
        ]}
      ]
    },

    {
      numero: 50,
      tipo: "cancion",
      titulo: "Bendice Alma Mía",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Bendice alma mía al Señor,",
          "No te olvides de sus beneficios.",
          "Acuérdate de dónde te sacó,",
          "Te sacó, te sacó.",
          "No te olvides que Él te libró,",
          "No te olvides que Él te salvó,",
          "Puso tus pies sobre peña",
          "Y una nueva canción te dio.",
          "Bendice alma mía al Señor,",
          "No te olvides de sus beneficios.",
          "Acuérdate de dónde te sacó,",
          "Te sacó, te sacó.",
          "No te olvides que Él te libró,",
          "No te olvides que Él te salvó,",
          "Puso tus pies sobre peña",
          "Y una nueva canción te dio."
        ]}
      ]
    },

    {
      numero: 51,
      tipo: "cancion",
      titulo: "Cuando Pienso en Tu Amor Tan Bello",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Cuando pienso en tu amor tan bello,",
          "Y te veo a tí en santidad,",
          "Y tu dignidad excede las riquezas de éste mundo.",
          "Cuando alcanzo a tu corazón,",
          "Cuando ya estoy en tu amor,",
          "Cuando todo alrededor es",
          "Sombras,",
          "Al lado de tu luz.",
          "Te adoro a tí, te adoro a tí,",
          "La razón por la cual yo vivo es para adorarte.",
          "Te adoro a tí, te adoro a tí,",
          "La razón por la cual yo vivo es para adorarte."
        ]}
      ]
    },

    {
      numero: 52,
      tipo: "cancion",
      titulo: "Sin Ti Señor",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Sin Ti Señor nada puedo ver,",
          "Si no estoy en Ti nada puedo hacer.",
          "Porque fuera de Ti no hay Dios,",
          "Porque en Ti tengo valor,"
        ]},
        { tipo: "coro", lineas: [
          "Sin Ti Señor ni siquiera puedo ser.",
          "Quiero estar unido a Ti",
          "Y muy fuerte permanecer.",
          "Arranca todo aquello que me hace inútil.",
          "Quiero estar unido a Ti y muy fuerte permanecer,",
          "Pués quiero que te sacies de mis frutos Señor."
        ]},
        { tipo: "estrofa", lineas: [
          "Como el pámpano si no está en la vid,",
          "Nada puede hacer ni frutos llevar.",
          "Sin tu fuego ya no hay calor,",
          "Sin tus brazos ya no hay amor,"
        ]},
        { tipo: "coro", lineas: [
          "Sin Ti Señor ni siquiera puedo ser.",
          "Quiero estar unido a Ti",
          "Y muy fuerte permanecer.",
          "Arranca todo aquello que me hace inútil.",
          "Quiero estar unido a Ti y muy fuerte permanecer,",
          "Pués quiero que te sacies de mis frutos Señor."
        ]}
      ]
    },

    {
      numero: 53,
      tipo: "cancion",
      titulo: "Él Volverá",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Como el relámpago,",
          "Que sale del oriente",
          "Y va al occidente,",
          "Así será, cuando vuelva el Señor.",
          "Como ladrón,",
          "Que viene en la noche",
          "Y nadie lo espera,",
          "Así será, cuando vuelva Jesús.",
          "Con voz de mando,",
          "Descenderá del cielo",
          "Y tocaran trompeta,",
          "Así será,",
          "Cuando vuelva el Señor.",
          "El amará,",
          "A los que han dormido",
          "Y a los que hemos creído,",
          "Así será cuando vuelva Jesús.",
          "El volvera,",
          "Y a su gloria me transformará,",
          "Regresara,",
          "Y en una nube me arrebatará.",
          "Con poder y gran gloria.",
          "Cristo Jesús, volverá."
        ]}
      ]
    },

    {
      numero: 54,
      tipo: "cancion",
      titulo: "Siglos en Oscuridad",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Siglos en oscuridad,",
          "Esperando una señal,",
          "La promesa cuándo llegará.",
          "En un campo de Belén,",
          "Se oye el llanto de un bebé y el silencio que una vez reinó.",
          "Lo rompió Emanuel.",
          "Aleluya, gloria en las alturas,",
          "Paz aquí en la tierra",
          "El Salvador nació.",
          "Aleluya gloria en las alturas,",
          "Gozo a las naciones Cristo el Señor.",
          "Admirable, consejero,",
          "Poderosos, príncipe de paz.",
          "Padre eterno, con nosotros,",
          "Luz del mundo, Cristo Emanuel.",
          "(Aleluya, Aleluya, Aleluya)",
          "Aleluya, gloria en las alturas,",
          "Gozo a las naciones,",
          "Cristo el Señor."
        ]}
      ]
    },

    {
      numero: 55,
      tipo: "cancion",
      titulo: "Reunidos Aquí",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Reunidos aquí para alabar al Señor,",
          "Nuevamente aquí en unidad.",
          "Reunidos aquí para alabar al Señor,",
          "Nuevamente aquí en unidad.",
          "Algo hermoso ha de acontecer,",
          "Algo tiene Dios para Ti.",
          "Reunidos aquí,",
          "Para alabar al Señor."
        ]}
      ]
    },

    {
      numero: 56,
      tipo: "cancion",
      titulo: "Él Vive Hoy",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "¿Por qué buscas en la tumba a aquel que vivo está?",
          "Recuerda su promesa me",
          "Levantaré.",
          "La piedra removida",
          "Su cuerpo no se halló",
          "Ha llegado el tercer día",
          "El Señor resucitó."
        ]},
        { tipo: "coro", lineas: [
          "Bendito sea el Dios",
          "Y Padre del Señor Jesús",
          "Quien vive hoy",
          "Esperanza viva tengo vida",
          "Junto a Él me dio"
        ]},
        { tipo: "estrofa", lineas: [
          "Y de la muerte me resucitó perfecto sacrificio el Padre recibió",
          "Segura garantía de nuestra justificación",
          "Cuando vuelva por su iglesia las tumbas se abrirán",
          "Y los que han dormido en Cristo con Él resucitarán"
        ]},
        { tipo: "coro", lineas: [
          "Bendito sea el Dios",
          "Y Padre del Señor Jesús",
          "Quien vive hoy",
          "Esperanza viva tengo vida",
          "Junto a Él me dio"
        ]},
        { tipo: "estrofa", lineas: [
          "Y de la muerte me resucitó ¿dónde está oh muerte tu aguijón",
          "Sepulcro dónde tu victoria?",
          "Si Cristo con su muerte te venció del pecado Él nos liberó",
          "Y ha reclamado su victoria la tumba no lo pudo retener",
          "Él vive hoy.",
          "Él vive hoy.",
          "Él vive hoy.",
          "Él vive hoy.",
          "Glorioso el día que de la muerte Él resucitó."
        ]}
      ]
    },

    {
      numero: 57,
      tipo: "cancion",
      titulo: "La Alegría",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "La alegría está en el corazón de aquel que conoce a Jesús.",
          "La verdadera paz la tiene aquel que ya conoce a Jesús."
        ]},
        { tipo: "coro", lineas: [
          "Un sentimiento tan hermoso que viene del Señor,",
          "Es el amor de aquel",
          "Que ya conoce a Jesús."
        ]},
        { tipo: "estrofa", lineas: [
          "¡Aleluya, Aleluya!",
          "¡Aleluya, Aleluya!"
        ]},
        { tipo: "coro", lineas: [
          "Un sentimiento tan hermoso que viene del Señor,",
          "Es el amor de aquel",
          "Que ya conoce a Jesús."
        ]}
      ]
    },

    {
      numero: 59,
      tipo: "cancion",
      titulo: "Dios Nos Envió",
      autor: "",
      tonalidad: "",
      referencia: "",
      etiquetas: [],
      bloques: [
        { tipo: "estrofa", lineas: [
          "Dios nos envió a su Hijo Cristo,",
          "Él es salud, paz y perdón.",
          "Vivió y murió por mis pecados,",
          "Vacía está la tumba porque Él triunfó."
        ]},
        { tipo: "coro", lineas: [
          "Porque Él vive triunfaré mañana,",
          "Porque Él vive ya no hay temor,",
          "Porque yo sé que el futuro es suyo,",
          "La vida vale más y más sólo por Él."
        ]},
        { tipo: "estrofa", lineas: [
          "Grato es tener a un tierno niño,",
          "Tocar su piel gozo nos da.",
          "Pero es mejor la dulce calma,",
          "Que Cristo el Rey nos puede dar,",
          "Pues vivo está."
        ]},
        { tipo: "coro", lineas: [
          "Porque Él vive triunfaré mañana,",
          "Porque Él vive ya no hay temor,",
          "Porque yo sé que el futuro es suyo,",
          "La vida vale más y más sólo por Él."
        ]},
        { tipo: "estrofa", lineas: [
          "Yo sé que un día el río cruzaré,",
          "Con el dolor, batallare.",
          "Y al ver la vida triunfando invicta,",
          "Veré gloriosas luces y veré al Rey."
        ]},
        { tipo: "coro", lineas: [
          "Porque Él vive triunfaré mañana,",
          "Porque Él vive ya no hay temor,",
          "Porque yo sé que el futuro es suyo,",
          "La vida vale más y más sólo por Él."
        ]}
      ]
    }

  ]
};
