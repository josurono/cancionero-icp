...
function paraLectura(item){
  ...
  /* Paso 1 — colapsar bloques IGUALES Y CONTIGUOS.
     Vienen de las marcas de repetición del original (/…/ = dos veces), y la
     proyección los necesita expandidos porque se cantan dos veces. Pero
     leyéndolos en el celular o en el papel, ver el mismo bloque dos veces
     seguidas es ruido: se muestra una vez con "×2".
     Ojo: contiguo NO es lo mismo que coro. Un coro alterna con las estrofas y
     sus apariciones quedan separadas; eso lo maneja el paso 2. */
  /* ¿El bloque entero es un tramo repetido N veces? Las marcas /…/ del
     original se expanden DENTRO del bloque (para que un tramo de una línea no
     termine siendo una diapositiva de una línea), así que la repetición queda
     acá adentro y no entre bloques. */
  function plegar(lineas){
    const n = lineas.length;
    for (let k = 1; k <= n / 2; k++){
      if (n % k !== 0) continue;
      let igual = true;
      for (let i = k; i < n && igual; i++){
        if (lineas[i] !== lineas[i % k]) igual = false;
      }
      if (igual) return { lineas: lineas.slice(0, k), veces: n / k };
    }
    return { lineas: lineas, veces: 1 };
  }

  const compactos = [];
  bloques.forEach(function(b){
    const crudas = lineasLimpias(b);
    if (!crudas.length) return;                 // bloque vacío: se ignora
    const tipo = b.tipo || 'estrofa';
    const pl = plegar(crudas);
    const firma = tipo + '\n' + pl.lineas.join('\n');
    const ult = compactos[compactos.length - 1];
    /* y si además dos bloques contiguos son idénticos, se suman */
    if (ult && ult.firma === firma){ ult.veces += pl.veces; return; }
    compactos.push({ tipo: tipo, lineas: pl.lineas, firma: firma, veces: pl.veces });
  });

  /* Paso 2 — numerar y marcar los coros que reaparecen más adelante. */
  const vistos = {};
  const salida = [];
  let nEstrofa = 0;

  compactos.forEach(function(b){
    const tipo = b.tipo, lineas = b.lineas;

    if (tipo === 'coro'){
      const f = lineas.join('\n');
      const n = distintos.indexOf(f) + 1;
      if (vistos[f]){
        salida.push({ clase:'alcoro', etiqueta: varios ? ('al coro ' + n) : 'al coro',
                      lineas: [], veces: b.veces });
      } else {
        vistos[f] = true;
        salida.push({ clase:'coro', etiqueta: varios ? ('Coro ' + n) : 'Coro',
                      lineas: lineas, veces: b.veces });
      }
      return;
    }
    if (tipo === 'estrofa'){
      nEstrofa++;
      salida.push({ clase:'estrofa', etiqueta:'Estrofa ' + nEstrofa,
                    lineas: lineas, veces: b.veces });
    } else {
